...
{
    public class FactoryHospitalTest
    {
        #region 医院列表
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public static bool EditHospital_Factory(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
                ...
                //编辑技工厂
                ComboBox cbFactory = appWin.MdiChild(SearchCriteria.ByAutomationId("AddOrEditHospitalView")).Get<ComboBox>(SearchCriteria.ByAutomationId("cbFactory"));
                cbFactory.Select(3);
                ...
                try
                {
                    ...
                    msg = "测试【编辑医院，修改技工厂】--通过，用时：" + (DateTime.Now - startTime).TotalSeconds;
                    return true;

                }
                catch
                {
                    msg = "测试【编辑医院，修改技工厂】--未通过，用时：" + (DateTime.Now - startTime).TotalSeconds;
                    return false;
                }
            }
            catch (Exception e)
            {
                msg = "测试【编辑医院，修改技工厂】--失败，原因：" + e.ToString();
                return false;
            }
        }
