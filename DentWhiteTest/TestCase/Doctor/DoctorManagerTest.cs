...
namespace DentWhiteTest.TestCase
{
    public class DoctorManagerTest
    {
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public static bool AddDoctor_IDCard(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        msg = "测试【新增医生，身份证不为空|身份证格式不正确】--通过，用时：" + (DateTime.Now - startTime).TotalSeconds;
                        return true;
                    }
                    catch
                    {
                        msg = "测试【新增医生，身份证不为空|身份证格式不正确】--未通过，缺少身份证格式验证提醒。用时：" + (DateTime.Now - startTime).TotalSeconds;
                        return false;
                    }
                }
                catch
                {
                    msg = "测试【新增医生，身份证不为空|身份证格式不正确】--未通过，缺少身份证空验证提醒。用时：" + (DateTime.Now - startTime).TotalSeconds;
                    return false;
                }
            }
            catch (Exception e)
            {
                msg = "测试【新增医生，身份证不为空|身份证格式不正确】--失败，原因：" + e.ToString();
                return false;
            }
        }
            
        /// <summary>
        /// 新增医生，手机号码为空|手机号码格式错误
        /// </summary>
        /// <param name="appWin"></param>
        /// <param name="msg"></param>
        /// <returns></returns>
        public static bool AddDoctor_Phone(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
                ...
                ////身份证为空
                //TextBox txtIDCard = appWin.MdiChild(SearchCriteria.ByAutomationId("AddOrEditDoctorView")).Get<TextBox>(SearchCriteria.ByAutomationId("txtIDCard"));
                //txtIDCard.Text = "440883199609080706";
        ...
        public static bool AddDoctor_HospitalNull(Window appWin, out string msg)
        {
            try
            {
                ...
                //身份证为空
                TextBox txtIDCard = appWin.MdiChild(SearchCriteria.ByAutomationId("AddOrEditDoctorView")).Get<TextBox>(SearchCriteria.ByAutomationId("txtIDCard"));
                txtIDCard.Text = "440889199909080706";
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public static bool EditDoctor_IDCard(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
        ...
        /// <summary>
        /// 编辑医生，手机号码为空|手机号码格式错误
        /// </summary>
        /// <param name="appWin"></param>
        /// <param name="msg"></param>
        /// <returns></returns>
        public static bool EditDoctor_Phone(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
        ...
        /// 点击匹配技师按钮，弹出提醒框，选择确定
        /// </summary>
        public static bool MatchingTechnician_Succ(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
                ...
                Random num = new Random();
                int i = num.Next(3, 10);//产生3到10的随机数

                //点击匹配技师按钮
                Button btnMatchTechnician = appWin.Get<Button>(SearchCriteria.ByAutomationId("btnMatchTechnician").AndIndex(5));
                btnMatchTechnician.Click();

                //选择技师
                ComboBox cbTechnician = appWin.Get<ComboBox>(SearchCriteria.ByAutomationId("cbTechnician"));
                cbTechnician.Select(1);

                //确认按钮
                Button btnConfirmMatchTechnician = appWin.Get<Button>(SearchCriteria.ByAutomationId("btnConfirmMatchTechnician"));
                btnConfirmMatchTechnician.Click();

                try
                {
                    //捕捉提醒信息
                    string error_info = appWin.Get<Label>(SearchCriteria.ByText("匹配成功！")).ToString();

                    //选择确定
                    Button btnTips = appWin.Get<Button>(SearchCriteria.ByAutomationId("2"));
                    btnTips.Click();

                   msg = "测试【匹配技师成功】--通过，用时：" + (DateTime.Now - startTime).TotalSeconds;
                   return true;
                    }
                 catch
                 {
                     msg = "测试【匹配技师成功】--未通过，缺少匹配成功提醒。用时：" + (DateTime.Now - startTime).TotalSeconds;
                     return false;
                 }
            }
            catch (Exception e)
            {
                msg = "测试【匹配技师成功】--失败，原因：" + e.ToString();
                return false;
            }
        }
        /// <summary>
        ///不选择技师，直接点击确定
        /// </summary>
        public static bool MatchingTechnician_Null(Window appWin, out string msg)
        {
            try
            {
                var startTime = DateTime.Now;
                ...
                Random num = new Random();
                int i = num.Next(11, 15);//产生3到10的随机数

                //点击匹配技师按钮
                Button btnMatchTechnician = appWin.Get<Button>(SearchCriteria.ByAutomationId("btnMatchTechnician").AndIndex(i));
                btnMatchTechnician.Click();
                ...
                {
                    //捕捉提醒信息
                    string error_info = appWin.Get<Label>(SearchCriteria.ByText("请选择技师！")).ToString();

                    //选择确定
                    Button btnTips = appWin.Get<Button>(SearchCriteria.ByAutomationId("2"));
                    btnTips.Click();

                    msg = "测试【不选择技师，点击确认】--通过，用时：" + (DateTime.Now - startTime).TotalSeconds;
                    return true;
                }
                catch
                {
                    msg = "测试【不选择技师，点击确认】--未通过，缺少选择技师提醒。用时：" + (DateTime.Now - startTime).TotalSeconds;
                    return false;
                }
            }
            catch (Exception e)
            {
                msg = "测试【不选择技师，点击确认】--失败，原因：" + e.ToString();
                return false;
            }
        }
        #endregion
